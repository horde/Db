...
use \Horde\Db\Adapter\Mysql\Result;
...
class Mysql extends Base
{
    ...
    public function connect()
    {
        ...
        if (!$mysql) {
            ...
            throw new DbException('Connect failed: ' . $error['message']);
        }
        if (!mysql_select_db($config['dbname'])) {
            throw new DbException('Could not select database: ' . $config['dbname']);
        }
    ...
    public function execute($sql, $arg1=null, $arg2=null)
    {
        ...
        if (!$stmt) {
            ...
            throw new DbException('QUERY FAILED: ' . mysql_error($this->_connection) . "\n\n" . $query,
